var crypto = require('crypto'),
	SmartPay = require('./smartpay_functions'),
...
TransactionService.prototype.buildParameterList = function (req, totalCost, email, callback) {
	...
	params.sessionValidity = moment(date).add(30,'minutes').format();
    params.shipBeforeDate = moment(date).add(30,'days').format();
...
TransactionService.prototype.buildSmartPayRequest = function (req, requestParameters) {
    return SmartPay.generateHmac(config.accounts[this.transaction.account].sharedKey, requestParameters);
};
TransactionService.prototype.getSmartPayURL = function () {
    return SmartPay.requestUrl();
};
...
TransactionService.prototype.buildSmartPayResponse = function (req, responseParameters) {
    delete responseParameters['merchantSig'];
    return SmartPay.generateHmac(config.accounts[this.transaction.account].sharedKey, responseParameters);
};
TransactionService.prototype.getVerifiedStatus = function (merchantSig, hmac) {
    return SmartPay.getVerifiedStatus(merchantSig, hmac);
};
			...
			if (document === undefined || document === null) {
				...
				inflateAndDecode(decryptedMerchantReturnData, function (merchantReturnDataDecoded) {
					...
					sendEmail(emailContents, function () {
						if (emailContents.emailTemplate === 'pay-legalisation-post-authorisation' && config.legalisationOfficeEmailAddress !== ""){
