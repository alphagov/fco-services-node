...
TransactionCalculator.prototype.calculate = function (values) {
  ...
  if (this.documentCount > 0 || this.registrationCount > 0){
      this.totalCost = (this.transaction['document_cost'] * this.documentCount);
  }else{
      this.totalCost = this.validatePrice(parseFloat(values['cost']).toFixed(2).toString());
  }
...
TransactionCalculator.prototype.validatePrice = function (price) {
    var isValid = validator.isFloat(price, { min: 0.00, max: 999.99 });
    if (isValid){
      return price;
    }else{
