...
TransactionCalculator.prototype.calculate = function (values) {
    ...
    this.cost = parseFloat(values['cost']) || 0;
    ...
    if (this.transaction.account === 'legalisation-post') {
        this.totalCost = this.validatePrice(parseFloat(this.cost).toFixed(2).toString());
    } else {
...
TransactionCalculator.prototype.validatePrice = function (price) {
    var isValid = validator.isFloat(price, {
        min: 0.01,
        max: 999.99
    });
    if (isValid) {
        ...
        throw new Error('Enter an amount between £0.01 and £999.99. Make two or more separate payments if you need to pay more than the maximum allowed at one time.');
    }
};
